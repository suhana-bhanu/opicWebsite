...
function Navbar() {
  const navAnimations = useRef(null);
  const navItemsRef = useRef([]);
  useEffect(() => {
    gsap.fromTo(
      navAnimations.current,
      { y: 0, opacity: 0 },
      { y: 20, opacity: 1, duration: 1.5, ease: "power2.out" }
    );
  }, []);
  ...
  return (
    <div
      ref={navAnimations}
      className="fixed z-[999] w-full flex justify-between items-center py-4 px-15 text-black"
    >
      <div className="logo">
        <svg
          width="72"
      ...
      <div ref={navAnimations} className="flex gap-7 text-black">
        {["Services", "Our Work", "About us", "Insights", "Contact Us"].map(
          (Item, index) => (
            <a
              key={index}
              ref={(el) => (navItemsRef.current[index] = el)}
              className={`relative text-regular capitalize gap-7 font-[NeueMontreal-Regular.ttf] ${
                index === 4 && "ml-32"
              }`}
            >
              {Item}
              <span className="absolute hover-line bottom-0 left-0 w-0 mt-2 h-[1px] bg-black"></span>
            </a>
          )
        )}
      </div>
